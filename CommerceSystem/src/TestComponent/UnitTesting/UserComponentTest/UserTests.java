...
//        store2 = CommerceService.getInstance().openStore("almog", "meatStore");
    ...
    public void openStore() {
        ...
        Assert.assertTrue(almog.getStoresOwned().contains(2));
    }
...
//    }
    ...
    public void addProductsToStoreState() {//todo add to purchase history
        ...
        keyword.add("bread");
        Collection<String> review = new LinkedList<>();
        Assert.assertFalse(yaakov.addProductsToStore(new ProductDTO("XL eggs", 0, 5, categories, keyword, review), 5).isFailure());
        Assert.assertTrue(yaakov.addProductsToStore(new ProductDTO("steak", 1, 5, categories, keyword, review), 5).isFailure());
    }

    @Test
    public void removeProductsFromStoreState() {//todo add to purchase history
        Assert.assertTrue(yaakov.removeProductsFromStore(1, -1, 5).isFailure());
        Assert.assertFalse(almog.removeProductsFromStore(1, -1, 5).isFailure());
    }
}

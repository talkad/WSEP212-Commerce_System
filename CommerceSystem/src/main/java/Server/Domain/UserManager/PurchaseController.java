...
public class PurchaseController {
         ...
         */
        public Response<List<PurchaseDTO>> handlePurchase(String bankAccount, ShoppingCart cart, String location) {
                ...
                if(!supplySystemAdapter.canDeliver(location, cart.getBaskets())) {
                        ...
                        return new Response<>(null, true, "Purchase Failed:\n" + "Delivery failed" + " | created external connection");
                }

                if(!paymentSystemAdapter.canPay(cart.getTotalPrice(), bankAccount)) {
                        storeController.addProductsToInventories(cart);
                        return new Response<>(null, true, "Purchase Failed:\n" + "Payment failed" + " | created external connection");
                }
